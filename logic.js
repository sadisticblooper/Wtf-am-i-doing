...
export class LogicHandler {
    ...
    async parse(arrayBuffer) {
        ...
        if (this.headerStart === -1) throw new Error('Invalid file signature');

        let offset = this.headerStart + 8;
        const arrayCount = dataView.getInt16(offset, true); offset += 2;
        offset += (arrayCount * 8); // Skip garbage
        ...
        this.animationDataEnd = this.headerEnd + (framesCount * this.frameSize);
        ...
        return {
            ...
            // Store raw trailing data for repacking later
            trailingData: arrayBuffer.slice(this.animationDataEnd)
        };
    }
    ...
    repack(animationData) {
        ...
        // Handle trailing data (allow for modified/multiplied trailing data from GLTF import)
        let footerBuffer = animationData.trailingData || new ArrayBuffer(0);
        ...
        for(let f = 0; f < framesCount; f++) {
            ...
            for(let b = 0; b < this.bonesCount; b++) {
                ...
                const packed = compressQuaternion(...boneData.rotation);
        ...
        if (footerBuffer.byteLength > 0) {
            finalBuffer.set(new Uint8Array(footerBuffer), writePtr);
        }
